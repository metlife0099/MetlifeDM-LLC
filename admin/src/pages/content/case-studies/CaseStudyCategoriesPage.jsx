...
export default function CaseStudyCategoriesPage() {
  ...
  const save = useMutation({
    mutationFn: (data) =>
      editOpen?._id ? caseStudiesApi.updateCategory(editOpen._id, data) : caseStudiesApi.createCategory(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'case-studies', 'categories'] });
  ...
  const remove = useMutation({
    mutationFn: (id) => caseStudiesApi.deleteCategory(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin', 'case-studies', 'categories'] });
  ...
  return (
    <>
      <Breadcrumbs items={[{ label: 'Content', href: '/content/case-studies' }, { label: 'Case Studies', href: '/content/case-studies' }, { label: 'Categories' }]} />
      <PageHeader
        eyebrow="Content / Case Studies / Categories"
        title={<>Case study <span className="text-italic-fraunces text-ultra">categories</span></>}
        subtitle="Shown as the category filter on the public case studies page."
      ...
      >
        <div className="space-y-4">
          ...
          <Input label="Slug" required prefix="/case-studies?category=" {...register('slug')} />
      ...
      <ConfirmDialog
        ...
        description="Case studies in this category will be uncategorized."
